...
func Authenticate(a contract.AuthService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if header == "" {
			c.AbortWithStatusJSON(http.StatusBadRequest, resterror.HttpResponseError{
				Err:     "bad_request",
		...
		if err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, resterror.HttpResponseError{
